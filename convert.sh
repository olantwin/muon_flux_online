...
inotifywait -r -m "$DIR" -e create -e moved_to |
	...
	[[ $(basename $FILE) =~ ^SPILLDATA ]] && sudo docker run -v $DIR:$DIR:Z olantwin/muon_flux_online:latest bin/bash -c "root -q -b \"unpack_tdc.C(\\\"$path$FILE\\\", \\\"$path$OUTPUTFILE\\\", $RUN)\" > $path$LOGFILE 2>&1"

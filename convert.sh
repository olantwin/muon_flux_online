...
inotifywait -r -m "$DIR" -e create -e moved_to |
	...
	xrdcp $OUTPUTFILE $EOSSHIP$OUTPUTPATH && rm $OUTPUTFILE && ./elog.py --text "Uploaded file $OUTPUTFILE of run $RUN; logfile: $HOSTNAME:$PWD/$LOGFILE" --subject "File upload run $RUN" --run $RUN
    done

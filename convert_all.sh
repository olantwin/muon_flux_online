...
for FILE in $DIR/*.rawdata; do
    ...
    xrdfs "$EOSSHIP" stat "$OUTPUTPATH"/"$FILE" || xrdcp "$DIR""$FILE" "$EOSSHIP""$OUTPUTPATH"
    xrdfs "$EOSSHIP" stat "$OUTPUTPATH"/"$OUTPUTFILE" || docker run -v "$DIR":"$DIR":Z -v "$PWD":/workdir:Z olantwin/muon_flux_online:$TAG bash -c "cd muon_flux_online; alienv setenv FairShip/latest -c root -q -b \"unpack_tdc.C(\\\"$DIR$FILE\\\", \\\"/workdir/$OUTPUTFILE\\\", $RUN)\" > /workdir/$LOGFILE 2>&1" && xrdcp "$OUTPUTFILE" "$EOSSHIP""$OUTPUTPATH" && rm -f "$OUTPUTFILE"
done
